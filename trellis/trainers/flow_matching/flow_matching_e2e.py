...
import json
from ..basic_ds import BasicTrainer

from ...pipelines import samplers 
from ...utils.general_utils import dict_reduce
from .mixins.classifier_free_guidance import ClassifierFreeGuidanceMixin, InterleaveClassifierFreeGuidanceMixin
...
from .mixins.interleave_conditioned import InterleaveConditionedMixin


from .mixins.interleave_conditioned_dino_clip import InterleaveConditionedDINOCLIPMixin

class FlowMatchingTrainerE2E(BasicTrainer):
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        model_root = os.path.join(decoder_root_path,decoder_model_name)
        cfg = edict(json.load(open(os.path.join(model_root, 'config.json'), 'r')))
    ...
    def training_losses(
        ...
        test_extrinsics: torch.Tensor = None,
        test_intrinsics: torch.Tensor = None,
        cond=None,
        **kwargs
    ) -> Tuple[Dict, Dict]:
        """
        ...
        """
        # select noise shape and random generalize it.
        ...
        pred = self.training_models['denoiser'](x_t, t * 1000, cond, **kwargs) # velocity of x_t at time t.
    ...
    def run_snapshot(
        ...
        for i in range(0, num_samples, batch_size):
            ...
            noise = torch.randn_like(data['x_0'].cuda())
            sample_gt.append(data['x_0'].cuda())
    ...
    def run_snapshot_test(
        ...
        if is_initialized():
            data_sampler = DistributedSampler(self.test_dataset)
        ...
        for data in dataloader:
            batch = data['x_0'].shape[0]
            data = recursive_to_device(data, torch.device("cuda"))
            extrinsics.append(data['test_extrinsics'])
            intrinsics.append(data['test_intrinsics'])
            images.append(data['test_image'])
            noise = torch.randn(batch, 16, 16, 16, 16).cuda()
        ...
        test_dict = {
            "images":{'value': images, 'type':"test"},
            "extrinsics":{'value': extrinsics, 'type':"test"},
            "intrinsics":{'value': intrinsics, 'type':"test"},
            # "condition_image":{'value':
        }
        sample_dict.update(test_dict)

        return sample_dict
class FlowMatchingCFGTrainerE2E(ClassifierFreeGuidanceMixin, FlowMatchingTrainerE2E):
    """
...
class TextConditionedFlowMatchingCFGTrainerE2E(TextConditionedMixin, FlowMatchingCFGTrainerE2E):
    """
...
class ImageConditionedFlowMatchingCFGTrainerE2E(ImageConditionedMixin, FlowMatchingCFGTrainerE2E):
    """
...
class InterleaveFlowMatchingCFGTrainerE2E(InterleaveClassifierFreeGuidanceMixin, FlowMatchingTrainerE2E):
    pass

class InterleaveConditionedDINOCLIPFlowMatchingCFGTrainerE2E(InterleaveConditionedDINOCLIPMixin, InterleaveFlowMatchingCFGTrainerE2E):
    pass