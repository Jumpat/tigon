...
class Trainer:
    ...
    @torch.no_grad()
    def snapshot(self, suffix=None, num_samples=64, batch_size=4, verbose=True):
        """
    ...
    def run(self):
        ...
        """
        try:
            if self.is_master and self.allow_snap:
                print('\nStarting training...')
                self.snapshot_dataset()
            if self.step == 0 and self.allow_snap:
                self.snapshot(suffix='init')
            elif  self.allow_snap: # resume
        ...
        while self.step < self.max_steps:
            ...
            # Sample images
            try:
                if self.step % self.i_sample == 0 and self.allow_snap:
                    self.snapshot()
            except:
                pass
            ...
            if self.is_master:
                ...
                # lora / nft
                if self.step % self.i_save == 0  and not self.fp16_mode == 'deepspeed':
                    self.save()

            # basic_ds!
            # Save checkpoint
            if self.step % self.i_save == 0 and self.fp16_mode == 'deepspeed':
                self.save()
