...
class SLatEncoderWithDownsampleV3(SparseTransformerBase):
    def __init__(
        self,
        resolution: int,
        in_channels: int,
        ...
        if preload_path != "":
            ...
            for name, params in self.named_parameters():
                ...
                if "downsample_network_feats" in name:
                    params.requires_grad = True    
        else:
    ...
    def forward(self, x: sp.SparseTensor, sample_posterior=True, return_raw=False, *args, **kwargs):
        h = super().forward(x)
        h = h.type(x.dtype)
        ...
        for data in z:
            ...
            # ss_voxels[:, 0, coords[:, 1], coords[:, 2], coords[:, 3]] = 1
...
class ElasticSLatEncoderWithDownsampleV3(SparseTransformerElasticMixin, SLatEncoderWithDownsampleV3):
    """
