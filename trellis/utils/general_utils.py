...
def nested_contexts(*contexts):
    ...
    Supports three forms:
      1) An instantiated context object (with __enter__/__exit__)
      2) A callable factory that returns a context object when invoked (such as DDP.no_sync or partial(...))
      3) A context class that is instantiated automatically (such as contextlib.nullcontext)
    ...
    with ExitStack() as stack:
        for ctx in contexts:
            ...
            # Case 3: a class; instantiate it directly
            ...
            # Case 1: already a context object
            if callable(enter) and callable(exit_):
                stack.enter_context(ctx)
                continue

            # Case 2: a callable factory; invoke it to obtain a context object
            if callable(ctx):
                cm = ctx()
                enter2 = getattr(cm, "__enter__", None)
                exit2 = getattr(cm, "__exit__", None)
                if not (callable(enter2) and callable(exit2)):
                    raise TypeError(f"Callable did not return a context manager: {ctx}")
