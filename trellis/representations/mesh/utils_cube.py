...
def find_features(valid_coords, h_coords, h_feats):
    ...
    # Build the mapping from unique_id to the row index in h_coords.
    ...
    # Use scatter to handle duplicates while keeping the first row index.
    map_id2row.scatter_(0, inv_coords.flip(0), row_ids.flip(0))
    
    # Find the row indices of valid entries in h_coords.
    idx_in_coords = map_id2row[inv_valid]  # [M]
    
    # Create the match mask.
